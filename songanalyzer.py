...
class Solution:
    def song_analyze(self,lyric):
        ...
        for x in range(len(words)):
            if words[x][0] in first_letters:
                if words[x][0] in alit_letters:
                    letter_count[alit_letters.index(words[x][0])] += 1
                else:
                    alit_letters.append(words[x][0])
                    letter_count.append(2)
            else:
                first_letters.append(words[x][0])
        for y in range(len(words)):
            if words[y][-3:] in word_endings:
                if words[y][-3:] in filtered_endings:
                    rhyme_amount[filtered_endings.index(words[y][-3:])] += 1
                else:
                    filtered_endings.append(words[y][-3:])
                    rhyme_amount.append(2)
            else:
                word_endings.append(words[y][-3:])
        for z in range(len(rhyme_amount)):
            rhyme_count+=rhyme_amount[z]
            
        string = ""
        for a in range(len(alit_letters)):
            string = string + "{letter}={number}, ".format(letter = alit_letters[a], number = letter_count[a]) 
        return string + "{rhymes} rhyming words".format(rhymes=rhyme_count)       
